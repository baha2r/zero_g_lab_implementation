...
class robotiqGymEnv(gym.Env):
    ...
    def reset(self):
        ...
        exttorqu = np.array([0,0,randt]) * (2 * self.targetmass)
        exttorqu = np.array([0,0,3]) * (2 * self.targetmass)
        # p.applyExternalForce(self.blockUid, -1 , extforce , [0,0,0] , p.LINK_FRAME)
        # p.applyExternalForce(self.blockUid, -1 , exttorqu , [0.05,0,0] , p.LINK_FRAME)
    ...
    def _is_success(self):
        ...
        Success is defined as having a reward greater than 2 for more than 250 consecutive steps.
        """
        if self.success_counter > 250:
            return 1.0
        ...
        return np.float32(self.success_counter > 250)
    ...
    def getExtendedObservation(self):
        ...
        gripEul = np.array(gripEul)
        # reduce self.tilt from gripEul[1] 
        gripEul[1] -= blockEul[1]
    ...
    def update_data(self):
        ...
        gripPos, gripOrn = p.getBasePositionAndOrientation(self._robotiq.robotiq_uid)
        gripEul = p.getEulerFromQuaternion(gripOrn)
        blockPos, blockOrn = p.getBasePositionAndOrientation(self.blockUid)
        blockEul = p.getEulerFromQuaternion(blockOrn)
        _, blockangVel = p.getBaseVelocity(self.blockUid)
        ...
        gripper_orientation = np.array(gripEul)
        ...
        block_orientation = np.array(blockEul)
        ...
        block_angular_velocity = np.array(blockangVel)
