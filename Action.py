...
from Observation import PoseListener
...
def get_observation():
    """Get the current observation from the PoseListener."""
    ...
    obs = np.zeros(39)
    obs[:3] = wall_arm_pose[0]
    obs[3:6] = wall_arm_pose[1]
    # obs[3:6] = np.zeros(3) # assume gripper orientation is zero
    # obs[4] = 1.23731041
    obs[6:9] = vel_wall[0]
    obs[9:12] = vel_wall[1]
    # obs[6:9] = action[0:3] * 1.13
    # obs[9:12] = action[3:6] * np.array([4.29, 9.83, 9.83])
    obs[12:15] = ceiling_arm_pose[0]
    obs[15:18] = ceiling_arm_pose[1]
    obs[15:18] = np.zeros(3) # assume target orientation is zero
    rel_pose = np.array(ceiling_arm_pose[0]) - np.array(wall_arm_pose[0])
    obs[18:21] = rel_pose
    rel_orientation = np.array(ceiling_arm_pose[1]) - np.array(wall_arm_pose[1])
    ...
    min_dist = min_distance_between_cuboids((wall_arm_pose[0], (0.05, 0.0518, 0.06)), 
                                            (ceiling_arm_pose[0], (0.1, 0.1, 0.02)))
    obs[36:39] = min_dist
    return obs
